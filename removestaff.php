...
<head>
    ...
    <link href='https://fonts.googleapis.com/css?family=Arvo' rel='stylesheet' type='text/css'>
     <style>
        ...
        #rem{
            color:black;
           transform:translate(510px,17.5px);  
        } 
       #rem1{
            color:red;
    ...
    <div id="contain">
    <div id="head">
        <h5 id="add"><span class="add1">Income Tax Calculator ->  </span>Home | Admin | Remove Staff<span class="back"><a href="admin1.php" class="link"><u>BACK</u></a></span></h5>
...
if(isset($_POST["submit"])){
 ...
 $sid=mysqli_real_escape_string($link,$sid);
    $sql="SELECT * from taxdetails where emp_id='$sid'";
        if($result=mysqli_query($link,$sql)){
           if(mysqli_num_rows($result)==0)
           {
            ...
            $sql1="SELECT * from taxdetails where emp_id='$sid' AND status='no'";
       ...
       if($x==1){
           $del1="UPDATE taxdetails set emp_id=null where emp_id='$sid'";
            if($result=mysqli_query($link,$del1)){
                $del2="DELETE from employee where emp_id='$sid'";
           if($result=mysqli_query($link,$del2)){
              $del3="DELETE from users where user_id='$sid'"; 
            if($result=mysqli_query($link,$del3)){
                echo "<p id='rem'>The employee has been successfully removed.</p>";
            }
           }
            }
       }
...
?>  
    ...
    <form method="post" action="removestaff.php">
     ...
     <br />
        <input type="submit" name="submit" class="btn btn-success btn-lg" id="button1" value="REMOVE" style="background-color:#e8bd78;">

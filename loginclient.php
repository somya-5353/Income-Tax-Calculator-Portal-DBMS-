...
<body> 
   ...
   <?php $link=@mysqli_connect("localhost","root","","tax") or die("Erro:Unable to connect: " . mysqli_connect_error());
    ...
    if(!$name || !$password)
    {
         ...
         setcookie("clientname",$name,time()+9600);
        $sql="SELECT client_id from client where name='$name' AND password='$password'";
    ...
    <form method="post" action="loginclient.php">
     ...
     <input type="submit" name="submit" class="btn btn-success btn-lg" id="button1" value="LOGIN" style="background-color:#e8bd78;">
        ...
        <p id="s1">Not a member? <a href="signupt.php" class="link"><u><i>Sign up now</i></u></a></p>
        </form>
    </div>    
    <script>
